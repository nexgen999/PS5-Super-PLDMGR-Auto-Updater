...
# =========================================================================
# 1. TRAITEMENT DES PAYLOADS (.ELF / .BIN) DEPUIS feed/*.opml
# =========================================================================
...
for opml_file in opml_files:
    ...
    for outline in outlines:
        ...
        # Filtre PS4 (conserve si le titre fait aussi référence à la PS5)
        if "ps4" in title.lower() and "ps5" not in title.lower():
            continue
        ...
        # Sources Fixes (.elf, .bin, .pkg, .zip)
        clean_xml_url = xml_url.split('?')[0].lower()
        if any(clean_xml_url.endswith(ext) for ext in ['.elf', '.bin', '.pkg', '.zip']):
            try:
                version = "Source-Fixe"
                ...
                with urllib.request.urlopen(req) as resp, open(os.path.join(target_dir, f_name), 'wb') as out_f:
        ...
        # Releases Forgejo (git.etawen.dev / Gitea)
...
# =========================================================================
# 2. TRAITEMENT DES PACKAGES (.PKG) DEPUIS PKGfeed/*.opml
# =========================================================================
