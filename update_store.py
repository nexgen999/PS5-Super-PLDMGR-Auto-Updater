...
for opml_file in opml_files:
    ...
    for outline in outlines:
        ...
        # Étape 1 : Détermination des nouveaux noms cibles sans renommer immédiatement
        rename_tasks = []
        for f_name in binaries_found:
            f_name_lower = f_name.lower()
            base_name, ext = os.path.splitext(f_name)
            
            # 1. RÈGLE DÉDIÉE : ZFTP / ZHTTP (Identification stricte sur le nom d'origine)
            ...
            # 3. RÈGLE DÉDIÉE : PS5-SELF-DECRYPTER
            ...
            rename_tasks.append((f_name, new_f_name))

        # Étape 2 : Application des renommages physiques sur le disque
        for old_f, new_f in rename_tasks:
            ...
            if old_path != new_path:
                try:
                    ...
                    print(f"   🏷️  Fichier renommé : {old_f} -> {new_f}")
                except Exception as rn_err:
                    print(f"   ⚠️ Erreur renommage : {rn_err}")
                    new_f = old_f
            
            if new_f not in eligible_binaries:
                eligible_binaries.append(new_f)
